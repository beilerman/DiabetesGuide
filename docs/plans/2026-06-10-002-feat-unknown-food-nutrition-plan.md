...
> **Decisions (Brad, 2026-06-11):** (1) beverage dosing-grade **approved** — `chain+decomposition` beverage → 72, `keyword+decomposition` beverage → 70; `keyword+chain` beverage stays 55 (fails the ≤1% undercount gate at 4.6%). (2) Disney-app calorie acquisition: "if possible" — verdict below. (3) The 10 E2 researched entries **approved and applied** (via `import-ai-nutrition --force`; the researched values supersede the inflated keyword-copy confidence labels). (4) PR #38 merged.
>
> **Disney-app verdict (decision 2):** the mobile-order calorie data sits behind an authenticated app session, and automated access to it violates Disney's Terms of Service — there is no TOS-clean "possible" here, so no scraper will be built for it. The TOS-safe paths remain: menu-board photo OCR (opportunistic, via the existing DFB photo scraper) and any future official/venue PDF publications. The E1 carb-fraction table stays ready for whenever official calories arrive by a legitimate route.
