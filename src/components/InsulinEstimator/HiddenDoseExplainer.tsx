...
export function HiddenDoseExplainer(props: HiddenDoseExplainerProps) {
  ...
  const srReasonsId = useId()
  const srReasonsText = `Dose hidden because: ${triggers.join(' ')}`

  if (triggers.length === 0) return null

  return (
    <>
      <div id={srReasonsId} role="note" aria-label={srReasonsText} className="sr-only" />
      <details
        className="mt-4 rounded-lg border border-stone-300 bg-white p-3 text-sm text-stone-700"
        aria-describedby={srReasonsId}
        open
      >
