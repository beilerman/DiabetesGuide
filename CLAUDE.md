...
`npm run loop:improve` runs a measure→act→re-measure→decide-until-converged loop that reuses the audit + enrichment building blocks to iteratively raise both **accuracy** (auto-fixes with confidence demotion) and **comprehensiveness** (prioritized enrichment chain: targeted → nutritionix → usda → edamam → triangulate → ai, where the in-process `targeted` arm fills the highest-priority genuine carb holes with corroborated keyword+chain estimates capped ≤65 and never overwrites existing carbs) of the nutrition DB. `control.ts` is pure (metrics + dosing-prioritized worklist + convergence); `improve.ts` orchestrates. Each iteration also runs an **accuracy back-test** (`accuracy-eval.ts`) that predicts official ground-truth carbs with the same keyword+chain method, leave-one-out, and reports dose-error (carb MAE, % within 10g, severe-undercount %, dose-units-off at ICR 10) — a distinct ACCURACY signal from coverage, reported per iteration and stored under each run's `accuracy` array in `loop-history.json` (not yet a convergence gate). It is measure/plan-only by default — **`--apply` is required for any DB write** — and appends metrics to `audit/loop-history.json` (`npm run loop:measure` / `--from-file` runs offline). Driven in CI by `.github/workflows/improvement-loop.yml` (weekly measure-only + manual dispatch with apply). **Canonical doc: [`scripts/loop/README.md`](scripts/loop/README.md).**
