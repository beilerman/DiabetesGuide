...
test('desktop sidebar uses shared navigation and hides the bottom bar', async ({ page }) => {
  ...
  await expect(sidebar.getByRole('link', { name: /^browse$/i })).toHaveAttribute('href', '/browse')
  await expect(sidebar.getByRole('link', { name: /^meal builder$/i })).toHaveAttribute('aria-current', 'page')
  await expect(sidebar.getByRole('link', { name: /^favorites$/i })).toHaveAttribute('href', '/plan')
  await expect(sidebar.getByRole('link', { name: /^menu$/i })).toHaveAttribute('href', '/more')
